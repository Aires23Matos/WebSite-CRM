...
const Clients = () => {
  ...
  const [blockDialog, setBlockDialog] = useState({ open: false, client: null, action: '' });
  ...
  const fetchBlockedClients = async () => {
    try {
      const response = await axios.get('http://localhost:3000/api/v1/client/blocked');
  ...
  const filteredActiveClients = filterClients(clients.filter(client => !client.isBlocked));
  ...
  const BlockedClientCard = ({ client }) => (
    <Card sx={{ mb: 2, p: 2, border: '1px solid', borderColor: 'error.light' }}>
      ...
      <Typography variant="caption" color="text.secondary" display="block" mb={2}>
        Bloqueado em: {client.cliente.blockedAt ? formatDate(client.cliente.blockedAt) : 'N/A'}
      </Typography>
    ...
    <div className="p-4 lg:p-6">
                        ...
                        <TableCell colSpan={7} align="center" sx={{ py: 4 }}>
                          {searchTerm ? (
                            <Box>
                              ...
                              <Typography variant="body2" color="gray.500" gutterBottom>
                                Não foram encontrados clientes para "{searchTerm}"
                              </Typography>
                              <Button 
                                variant="outlined" 
          ...
          <Box>
            {filteredBlockedClients.length === 0 ? (
                ...
                <CardContent sx={{ textAlign: 'center', py: 4 }}>
                  {searchTerm ? (
                    <>
                      ...
                      <Typography variant="body2" color="gray.500">
                        Não foram encontrados clientes bloqueados para "{searchTerm}"
                      </Typography>
                      <Button 
                        variant="outlined" 
                        onClick={clearSearch}
              ...
              filteredBlockedClients.map((client) => (
                <BlockedClientCard key={client.cliente.client_id} client={client} />
              ))
            )}
          </Box>
        ) : (
                ...
                <Table>
                  <TableHead>
                    <TableRow sx={{ backgroundColor: '#fef2f2' }}>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>Cliente</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>NIF</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>Motivo do Bloqueio</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>Data do Bloqueio</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>Contactos</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151' }}>Endereços</TableCell>
                      <TableCell sx={{ fontWeight: 'bold', color: '#374151', textAlign: 'center' }}>Ações</TableCell>
                    </TableRow>
                    ...
                    {filteredBlockedClients.length === 0 ? (
                      ...
                      filteredBlockedClients.map((client) => (
                        ...
                        >
                          ...
                          <TableCell>
                            <Typography variant="body2">{client.cliente.nif}</Typography>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2" color="error.main">
                              {client.cliente.blockReason || 'Sem motivo especificado'}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2">
                              {client.cliente.blockedAt ? formatDate(client.cliente.blockedAt) : 'N/A'}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
      ...
      >
        <DialogTitle id="block-dialog-title">
          {blockDialog.action === 'block' ? 'Bloquear Cliente' : 'Desbloquear Cliente'}
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
        ...
        <DialogActions>
          ...
          <Button 
            ...
            startIcon={blockDialog.action === 'block' ? <BlockIcon /> : <UnlockIcon />}
          >
            {blockLoading ? 'Processando...' : (blockDialog.action === 'block' ? 'Bloquear' : 'Desbloquear')}
          </Button>
        </DialogActions>
      </Dialog>
